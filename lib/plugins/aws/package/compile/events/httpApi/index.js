...
Object.defineProperties(
  ...
  memoizeeMethods({
    resolveConfiguration: d(function() {
      ...
      this.config = { routes, id: userConfig.id, payloadFormatVersion: '1.0' };

      if (userConfig.payloadFormatVersion) {
        if (!['1.0', '2.0'].includes(userConfig.payloadFormatVersion)) {
          throw new this.serverless.classes.Error(
            'provider.httpApi.payloadFormatVersion must be either "1.0" or "2.0"',
            'EXTERNAL_HTTP_API_PAYLOAD_FORMAT_VERSION_CONFIG'
          );
        }

        this.config.payloadFormatVersion = userConfig.payloadFormatVersion;
      }
    ...
    compileIntegration: d(function(routeTargetData) {
      const properties = {
        ...
        PayloadFormatVersion: this.config.payloadFormatVersion,
      };
      if (routeTargetData.timeout) {
